...
if ($rowr[0] > 0) {
	...
	foreach ($cdr_records as $cdr_record) {
		...
		$values = mysql_query("SELECT result, attempts, lastattempt FROM dialout_numbers WHERE result='".$_GET['type']."' AND prov!='TEST' AND projectid=".$cdr_record['projectid']." AND phonenumber='".$cdr_record['phonenumber']."'") OR die("error executing if inside SQL");
...
mysql_close($con);
$file = $_GET['projectid']."_detail_report_for_".$_GET['type'];
